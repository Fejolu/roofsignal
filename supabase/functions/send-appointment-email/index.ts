...
serve(async (req) => {
  ...
  const { data: appointment, error } = await service.from("appointments")
    .select("id,organization_id,property_id,title,starts_at,ends_at,notes,inspector_id")
    .eq("id", appointmentId).single();
  if (error || !appointment) return new Response(JSON.stringify({ error: error?.message || "Afspraak niet gevonden." }), { status: 404, headers: cors });
  const organization = appointment.organization_id
    ? (await service.from("organizations").select("name,contact_name,contact_email").eq("id", appointment.organization_id).maybeSingle()).data
    : null;
  const property = appointment.property_id
    ? (await service.from("properties").select("name,address,postcode,city").eq("id", appointment.property_id).maybeSingle()).data
    : null;
  ...
  for (const message of recipients) {
    ...
    const result = await sendBrevo({ sender, to: [{ email: message.email, name: message.name }], ...(message.email==="ferry@roofsignal.nl"?{}:{bcc:[{email:"ferry@roofsignal.nl",name:"F.J. Joosten"}]}), replyTo, headers: { "X-Mailin-Track": "0" }, subject: `${testRecipient ? "[TEST] " : ""}RoofSignal afspraakbevestiging – ${property?.name || address}`, textContent: text, htmlContent: html, attachment });
