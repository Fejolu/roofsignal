...
serve(async (req) => {
  ...
  for (const message of recipients) {
    ...
    const result = await sendBrevo({ sender, to: [{ email: message.email, name: message.name }], replyTo, headers: { "X-Mailin-Track": "0" }, subject: `${testRecipient ? "[TEST] " : ""}RoofSignal afspraakbevestiging – ${property?.name || address}`, textContent: text, htmlContent: html, attachment });
