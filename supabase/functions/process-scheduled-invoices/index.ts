...
serve(async(req)=>{
  ...
  if(body?.prepareInternalTest===true){
    const recipient=String(body?.recipient||"").trim().toLowerCase();
    if(!/^[^@\s]+@roofsignal\.nl$/.test(recipient))return new Response(JSON.stringify({error:"Interne testontvanger moet op @roofsignal.nl eindigen."}),{status:400,headers});
    const orgResult=await service.from("organizations").select("id").ilike("name","%Interne E2E-test%").limit(1).maybeSingle();
    if(orgResult.error||!orgResult.data)return new Response(JSON.stringify({error:"Interne E2E-testorganisatie ontbreekt."}),{status:409,headers});
    const created=await service.from("invoices").insert({organization_id:orgResult.data.id,amount:1,status:"draft",due_date:new Date(Date.now()+14*86400000).toISOString().slice(0,10),payment_url:"https://www.roofsignal.nl/?interne-factuurtest=1",bank_account:"NL35INGB0700762019",account_holder:"FJ Joosten",payment_term_days:14,auto_send_at:new Date().toISOString(),auto_send_status:"scheduled",is_test:true,test_recipient:recipient}).select("id").single();
    if(created.error)return new Response(JSON.stringify({error:created.error.message}),{status:500,headers});
    const line=await service.from("invoice_lines").insert({invoice_id:created.data.id,description:"Interne E2E-test factuurroute - niet betalen",quantity:1,unit_price:1,vat_rate:21});
    if(line.error){await service.from("invoices").delete().eq("id",created.data.id);return new Response(JSON.stringify({error:line.error.message}),{status:500,headers});}
    return new Response(JSON.stringify({ok:true,prepared:true}),{headers});
  }
  let queueQuery=service.from("invoices").select("*,organizations(name,contact_name,contact_email)").in("auto_send_status",["scheduled","failed"]).lte("auto_send_at",new Date().toISOString()).eq("status","draft");
  queueQuery=body?.testOnly===true?queueQuery.eq("is_test",true):queueQuery.eq("is_test",false);
  const {data:queue,error}=await queueQuery.order("auto_send_at").limit(20);
  ...
  for(const invoice of queue||[]){
    ...
    try{
      ...
      const recipient=String(invoice.is_test?invoice.test_recipient:invoice.organizations?.contact_email||"").trim().toLowerCase();if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(recipient))throw new Error("Geldig klant-e-mailadres ontbreekt.");
      if(invoice.is_test&&!/@roofsignal\.nl$/.test(recipient))throw new Error("Interne test mag alleen naar @roofsignal.nl.");
      ...
      const testNotice=invoice.is_test?`<div style="background:#fff2ed;border:2px solid #ff5a1f;padding:14px 16px;margin:0 0 22px;font-weight:800">INTERNE E2E-TEST — NIET BETALEN</div>`:"";
      const html=`<!doctype html><html><body style="margin:0;background:#f3f5f4;font-family:Arial,sans-serif;color:#17201d"><table width="100%" style="padding:28px 12px"><tr><td align="center"><table width="100%" style="max-width:600px;background:#fff;border-radius:12px;overflow:hidden"><tr><td style="background:#101715;padding:24px 28px;color:#fff;font-size:20px;font-weight:800">ROOF<span style="color:#ff5a1f">SIGNAL</span></td></tr><tr><td style="padding:34px 32px">${testNotice}<div style="color:#ff5a1f;font-size:12px;font-weight:800;letter-spacing:1.2px">FACTURATIE</div><h1>${invoice.is_test?"Testfactuur":"Uw factuur"}</h1><p>${esc(salutation)}</p><p>In de bijlage vindt u de factuur voor de uitgevoerde RoofSignal-opdracht.</p><div style="background:#f6f7f6;border-left:4px solid #ff5a1f;padding:16px 18px;margin:22px 0"><strong>${esc(detail)}</strong></div><p style="margin:24px 0"><a href="${esc(invoice.payment_url)}" style="display:inline-block;background:#ff5a1f;color:#fff;text-decoration:none;font-weight:800;padding:14px 24px;border-radius:8px">${invoice.is_test?"Test betaallink":"Betaal nu"}</a></p><p>Met vriendelijke groet,<br><strong>F.J. Joosten</strong><br>RoofSignal</p></td></tr></table></td></tr></table></body></html>`;
      const sent=await brevo({sender:{email:Deno.env.get("BREVO_FROM_EMAIL")||"noreply@roofsignal.nl",name:"RoofSignal"},to:[{email:recipient,name:invoice.is_test?"RoofSignal interne test":invoice.organizations?.contact_name||invoice.organizations?.name}],...(recipient==="ferry@roofsignal.nl"?{}:{bcc:[{email:"ferry@roofsignal.nl",name:"F.J. Joosten"}]}),replyTo:{email:"info@roofsignal.nl",name:"RoofSignal"},subject:`${invoice.is_test?"[TEST] ":""}Uw factuur ${invoice.invoice_number||"RoofSignal"}`,htmlContent:html,textContent:`${invoice.is_test?"INTERNE E2E-TEST - NIET BETALEN\n\n":""}${salutation}\n\nIn de bijlage vindt u uw factuur.\n${detail}\n${invoice.is_test?"Test betaallink":"Betaal nu"}: ${invoice.payment_url}\n\nMet vriendelijke groet,\nF.J. Joosten\nRoofSignal`,attachment:[{name:document.title.endsWith(".pdf")?document.title:`${document.title}.pdf`,content:b64(new Uint8Array(await download.data.arrayBuffer()))}]});
